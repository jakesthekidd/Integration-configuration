...
@Component({
  ...
  styles: [
    `
      ...
      .stat-card.partial .stat-num {
        color: #8e44ad;
      }
      .stat-card.error .stat-num {
        color: #e74c3c;
      }
      .stat-card.neutral .stat-num {
        color: #3498db;
      }
...
export class TransformationLogsComponent implements OnInit {
  ...
  load() {
    ...
    this.error = '';
    this.api
      .getTransformationLogs(this.filterTemplateId || undefined, this.filterStatus || undefined, this.filterLimit)
      .subscribe({
        next: (res) => {
